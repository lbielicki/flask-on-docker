...
def get_input_stream(
    ...
    # that case the stream is safe without wrapping, or can enforce a max
    # length.
...
class LimitedStream(io.RawIOBase):
    ...
    def __init__(self, stream: t.IO[bytes],
                 limit: int, is_max: bool = False) -> None:
        self._stream = stream
        self._pos = 0
        self.limit = limit
    ...
    def readinto(self, b: bytearray) -> int | None:  # type: ignore[override]
        ...
        if hasattr(self._stream, "readinto"):
            ...
            if size <= remaining:
                # The size fits in the remaining limit, use the buffer
                # directly.
                try:
    ...
    def readall(self) -> bytes:
        ...
        # The parent implementation uses "while True", which results in an
        # extra read.
